...
from .validators import (
    valider_rappel_complet, 
    obtenir_conseils_optimisation,
    RappelTypeValidator,
...
class RappelCreateView(StaffRequiredMixin, CreateView):
    """
    Vue pour créer un rappel avec support de la planification.
    """
    model = Rappel
    ...
    def get_context_data(self, **kwargs):
        ...
        if cotisation_id:
            try:
                ...
                logger.error(f"Cotisation {cotisation_id} non trouvée")
        
        context['today'] = timezone.now().date()
        return context
    
    def get_form_kwargs(self):
        """Passer les données supplémentaires au formulaire."""
        kwargs = super().get_form_kwargs()
        ...
        if cotisation_id:
            try:
                cotisation = Cotisation.objects.get(pk=cotisation_id)
                kwargs['cotisation'] = cotisation
                kwargs['membre'] = cotisation.membre
            except Cotisation.DoesNotExist:
                logger.error(f"Cotisation {cotisation_id} non trouvée")
    ...
    def form_valid(self, form):
        ...
        # Message de succès personnalisé selon le type d'envoi
        rappel = form.instance
        if rappel.etat == 'planifie':
            message = _("Le rappel a été créé et sera envoyé le {} à {}.").format(
                rappel.date_envoi.strftime('%d/%m/%Y'),
                rappel.date_envoi.strftime('%H:%M')
            )
        else:
            message = _("Le rappel a été créé et envoyé immédiatement.")
        
        messages.success(self.request, message)
        
        # Log pour suivi
        logger.info(f"Rappel créé par {self.request.user}: ID={rappel.id}, État={rappel.etat}")
        
        return response
    
    def get_success_url(self):
        """Rediriger vers la page de détail de la cotisation."""
        cotisation_id = self.kwargs.get('cotisation_id')
        if cotisation_id:
            return reverse_lazy('cotisations:cotisation_detail', kwargs={'pk': cotisation_id})
        return reverse_lazy('cotisations:rappel_liste')
    
    def form_invalid(self, form):
        """Gestion des erreurs de formulaire."""
        logger.warning(f"Erreurs dans le formulaire de rappel: {form.errors}")
        
        # Ajouter un message d'erreur global
        messages.error(
            self.request, 
            _("Erreur lors de la création du rappel. Veuillez vérifier les informations saisies.")
        )
...
def rappel_create_ajax(request, cotisation_id):
    ...
    Utilise maintenant le même RappelForm pour la cohérence.
    """
    cotisation = get_object_or_404(Cotisation, pk=cotisation_id)
    ...
    if request.content_type == 'application/json':
        try:
            ...
            # Convertir les données JSON au format attendu par RappelForm
            form_data = {
            ...
            # Traiter les options d'envoi selon le nouveau format
            planifie = data.get('planifie') == 'true'
            date_planifiee = data.get('date_planifiee')
            
            # Ajouter les champs de planification au format attendu par RappelForm
            form_data['envoi_immediat'] = not planifie  # Inverser la logique
            
            if planifie and date_planifiee:
                try:
                    # Parser la date/heure ISO
                    date_planifiee = str(date_planifiee).strip()
                    
                    # Nettoyer la date
                    if '+' in date_planifiee:
                        date_planifiee = date_planifiee.split('+')[0]
                    if 'Z' in date_planifiee:
                        date_planifiee = date_planifiee.replace('Z', '')
                    
                    # Ajouter les secondes si manquantes
                    if len(date_planifiee.split('T')[1]) == 5:
                        date_planifiee += ':00'
                    
                    # Parser la date et l'heure
                    date_obj = datetime.datetime.fromisoformat(date_planifiee)
                    
                    # Séparer date et heure pour le formulaire
                    form_data['date_planifiee'] = date_obj.date()
                    form_data['heure_planifiee'] = date_obj.time()
                    
                    logger.info(f"Date planifiée parsée: {date_obj}")
                    
                except (ValueError, TypeError) as e:
                    logger.error(f"Erreur de format de date: {str(e)} - Date reçue: {date_planifiee}")
                    return JsonResponse({
                        'success': False,
                        'message': _("Format de date invalide: {}").format(str(e))
                    }, encoder=ExtendedJSONEncoder, status=400)
            
            # Validation des données essentielles
            if not form_data.get('type_rappel'):
                return JsonResponse({
                    'success': False,
                    'message': _("Le type de rappel est obligatoire")
                }, encoder=ExtendedJSONEncoder, status=400)
            
            if not form_data.get('contenu'):
                return JsonResponse({
                    'success': False,
                    'message': _("Le contenu du rappel est obligatoire")
                }, encoder=ExtendedJSONEncoder, status=400)
            
            try:
                niveau = int(form_data.get('niveau', 1))
                if niveau < 1 or niveau > 10:
                    return JsonResponse({
                        'success': False,
                        'message': _("Le niveau doit être entre 1 et 10")
                    }, encoder=ExtendedJSONEncoder, status=400)
                form_data['niveau'] = niveau
            except (ValueError, TypeError):
                return JsonResponse({
                    'success': False,
                    'message': _("Le niveau doit être un nombre valide")
                }, encoder=ExtendedJSONEncoder, status=400)
            
            # Utiliser le RappelForm pour la cohérence
            user = request.user if request.user.is_authenticated else None
            ...
            if form.is_valid():
                ...
                # Log pour suivi
                logger.info(f"Rappel créé via AJAX par {user}: ID={rappel.id}, État={rappel.etat}")
                
                # Message de succès personnalisé selon le type d'envoi
                if rappel.etat == 'planifie':
                    success_message = _("Le rappel a été créé et sera envoyé le {} à {}.").format(
                        rappel.date_envoi.strftime('%d/%m/%Y'),
                        rappel.date_envoi.strftime('%H:%M')
                    )
                else:
                    success_message = _("Le rappel a été créé et envoyé immédiatement.")
                ...
                for field, error_list in form.errors.items():
                    # Adapter les noms de champs pour l'affichage côté client
                    display_field = field
                    if field == 'date_planifiee':
                        display_field = 'date_planifiee'
                    elif field == 'heure_planifiee':
                        display_field = 'heure_planifiee'
                    
                    errors[display_field] = [{'message': str(error), 'code': ''} for error in error_list]
                
                # Log pour débogage
                logger.warning(f"Erreurs de validation AJAX: {form.errors}")
                logger.warning(f"Données reçues: {form_data}")
            ...
            logger.error(f"Erreur inattendue dans rappel_create_ajax: {str(e)}")
            logger.error(traceback.format_exc())
            return JsonResponse({
                'success': False,
                'message': _("Erreur serveur inattendue")
            }, encoder=ExtendedJSONEncoder, status=500)
    
    # Pour les requêtes standard (non-AJAX) - utiliser le même format
    else:
        # Convertir les données POST au format RappelForm si nécessaire
        post_data = request.POST.copy()
        
        user = request.user if request.user.is_authenticated else None
        form = RappelForm(post_data, user=user, cotisation=cotisation, membre=membre)
        ...
        if form.is_valid():
            ...
            errors = {}
            for field, error_list in form.errors.items():
                errors[field] = [{'message': str(error), 'code': ''} for error in error_list]
...
class RappelDeleteView(StaffRequiredMixin, DeleteView):
    """
    Vue pour supprimer un rappel planifié (non envoyé).
    """
    model = Rappel
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['cotisation'] = self.object.cotisation
        # Ajouter une variable pour indiquer si le rappel peut être supprimé
        context['can_delete'] = self.object.etat == RAPPEL_ETAT_PLANIFIE
        return context
    
    def dispatch(self, request, *args, **kwargs):
        """Vérifie si le rappel peut être supprimé avant de traiter la requête"""
        self.object = self.get_object()
        
        # Vérifier que le rappel est bien en état planifié
        if self.object.etat != RAPPEL_ETAT_PLANIFIE:
            messages.error(
                request,
                _("Seuls les rappels planifiés peuvent être supprimés.")
            )
            return redirect('cotisations:rappel_detail', pk=self.object.pk)
        
        return super().dispatch(request, *args, **kwargs)
    
    def form_valid(self, form):
        """Appelé lorsque le formulaire est valide (après POST)"""
        cotisation = self.object.cotisation
        messages.success(
            self.request, 
            _("Le rappel a été supprimé avec succès.")
        )
        # La suppression sera effectuée par la méthode delete() de DeleteView
        response = super().form_valid(form)
        return response

    def get_success_url(self):
        """URL de redirection après la suppression réussie"""
        cotisation_id = self.object.cotisation.id
        return reverse('cotisations:cotisation_detail', kwargs={'pk': cotisation_id})
#
...
@login_required
def api_rappel_templates(request):
    """
    API pour récupérer les templates de rappel disponibles.
    """
    type_rappel = request.GET.get('type_rappel', 'email')
    niveau = int(request.GET.get('niveau', 1))
    langue = request.GET.get('langue', 'fr')
    cotisation_id = request.GET.get('cotisation_id')
    
    # Récupérer les templates disponibles
    templates = RappelTemplate.get_templates_pour_type(
        type_rappel=type_rappel,
        niveau=niveau,
        langue=langue
    )
    ...
    # Si une cotisation est spécifiée, préparer le contexte pour le rendu
    cotisation = None
    if cotisation_id:
        try:
            cotisation = Cotisation.objects.get(pk=cotisation_id)
    ...
    for template in templates:
        template_data = {
            ...
            'niveau_min': template.niveau_min,
            'niveau_max': template.niveau_max,
        }
        
        # Si on a une cotisation, générer le contenu personnalisé
        if cotisation:
            try:
                template_data['contenu_genere'] = template.generer_contenu(cotisation)
                template_data['sujet_genere'] = template.sujet
                
                # Remplacer les variables dans le sujet aussi
                variables = {
                ...
                sujet_genere = template.sujet
                for var, valeur in variables.items():
                    sujet_genere = sujet_genere.replace(f'{{{var}}}', str(valeur))
                template_data['sujet_genere'] = sujet_genere
                
            except Exception as e:
                logger.error(f"Erreur lors de la génération du contenu: {str(e)}")
                template_data['contenu_genere'] = template.contenu
                template_data['sujet_genere'] = template.sujet
        
        templates_data.append(template_data)
    
    # Si aucun template trouvé, créer les templates par défaut
    if not templates_data:
        try:
            RappelTemplate.creer_templates_par_defaut()
            # Réessayer
            templates = RappelTemplate.get_templates_pour_type(
                type_rappel=type_rappel,
                niveau=niveau,
            ...
            logger.error(f"Erreur lors de la création des templates par défaut: {str(e)}")
...
@login_required
def api_rappel_template_detail(request, template_id):
    """
    API pour récupérer le détail d'un template de rappel spécifique.
    """
    try:
        template = RappelTemplate.objects.get(pk=template_id, actif=True)
        cotisation_id = request.GET.get('cotisation_id')
        ...
        # Si une cotisation est spécifiée, générer le contenu personnalisé
        if cotisation_id:
            try:
                cotisation = Cotisation.objects.get(pk=cotisation_id)
                template_data['contenu_genere'] = template.generer_contenu(cotisation)
                
                # Générer le sujet personnalisé
                variables = {
                    'association_nom': getattr(settings, 'ASSOCIATION_NAME', 'Notre Association'),
                    'reference': cotisation.reference,
                    'prenom': cotisation.membre.prenom,
    ...
    except RappelTemplate.DoesNotExist:
        return JsonResponse({
            'success': False,
            'message': _("Template de rappel non trouvé.")
        }, encoder=ExtendedJSONEncoder, status=404)
    except Exception as e:
...
@login_required
@require_POST
def api_rappel_template_previsualiser(request):
    """
    API pour prévisualiser un template de rappel avec une cotisation donnée.
    """
    try:
        data = json.loads(request.body)
        template_id = data.get('template_id')
        cotisation_id = data.get('cotisation_id')
        contenu_custom = data.get('contenu_custom')  # Contenu personnalisé à la volée
        
        if not cotisation_id:
            return JsonResponse({
                'success': False,
                'message': _("ID de cotisation requis pour la prévisualisation.")
            }, encoder=ExtendedJSONEncoder, status=400)
        
        cotisation = Cotisation.objects.get(pk=cotisation_id)
        
        # Utiliser un template existant ou du contenu personnalisé
        if template_id:
            template = RappelTemplate.objects.get(pk=template_id, actif=True)
            contenu_genere = template.generer_contenu(cotisation)
            sujet_genere = template.sujet
        elif contenu_custom:
            # Créer un template temporaire pour générer le contenu
            temp_template = RappelTemplate(
                contenu=contenu_custom,
                sujet=data.get('sujet_custom', '')
            )
            contenu_genere = temp_template.generer_contenu(cotisation)
            sujet_genere = temp_template.sujet
        else:
            return JsonResponse({
                'success': False,
                'message': _("Template ID ou contenu personnalisé requis.")
            }, encoder=ExtendedJSONEncoder, status=400)
    ...
    except (Cotisation.DoesNotExist, RappelTemplate.DoesNotExist) as e:
        return JsonResponse({
            'success': False,
            'message': _("Ressource non trouvée: {}").format(str(e))
        }, encoder=ExtendedJSONEncoder, status=404)
    except Exception as e:
...
# Vue pour l'administration des templates (optionnelle)
class RappelTemplateListView(StaffRequiredMixin, ListView):
    """
    Vue pour lister et gérer les templates de rappel.
    """
    model = RappelTemplate
    template_name = 'cotisations/template_rappel_liste.html'
    context_object_name = 'templates'
    paginate_by = 20
    
    def get_queryset(self):
        queryset = RappelTemplate.objects.all()
        
        # Filtres
        type_template = self.request.GET.get('type_template')
        type_rappel = self.request.GET.get('type_rappel')
        ...
        return queryset.order_by('type_template', 'ordre_affichage', 'nom')


class RappelTemplateCreateView(StaffRequiredMixin, CreateView):
    """
    Vue pour créer un nouveau template de rappel.
    """
    model = RappelTemplate
    fields = [
        'nom', 'type_template', 'type_rappel', 'sujet', 'contenu',
        'langue', 'actif', 'niveau_min', 'niveau_max', 'ordre_affichage',
        'variables_supplementaires'
    ]
    template_name = 'cotisations/template_rappel_form.html'
    success_url = reverse_lazy('cotisations:template_rappel_liste')
    
    def form_valid(self, form):
        messages.success(self.request, _("Template de rappel créé avec succès."))
        return super().form_valid(form)


class RappelTemplateUpdateView(StaffRequiredMixin, UpdateView):
    """
    Vue pour modifier un template de rappel existant.
    """
    model = RappelTemplate
    fields = [
        'nom', 'type_template', 'type_rappel', 'sujet', 'contenu',
    ...
    def form_valid(self, form):
        messages.success(self.request, _("Template de rappel modifié avec succès."))
        return super().form_valid(form)

@login_required
@require_http_methods(["GET"])
def api_contraintes_type_rappel(request):
    """
    API pour récupérer les contraintes d'un type de rappel.
    """
    type_rappel = request.GET.get('type_rappel', 'email')
    
    try:
        # Récupérer la configuration
        contraintes = TypeRappelConfig.get_contraintes_ui(type_rappel)
        variables = TypeRappelConfig.get_variables_disponibles(type_rappel)
        
        # Estimation de coût
        nb_destinataires = int(request.GET.get('nb_destinataires', 1))
        cout_estime = TypeRappelConfig.estimer_cout(type_rappel, nb_destinataires)
        ...
        return JsonResponse({
            ...
            'niveau_recommande': TypeRappelConfig.get_niveau_recommande(
                type_rappel, 
                int(request.GET.get('jours_retard', 0))
            )
        }, encoder=ExtendedJSONEncoder)
        
    except Exception as e:
        logger.error(f"Erreur lors de la récupération des contraintes: {str(e)}")
        return JsonResponse({
            'success': False,
            'message': str(e)
        }, encoder=ExtendedJSONEncoder, status=500)
...
def api_valider_contenu_rappel(request):
    """
    API pour valider le contenu d'un rappel en temps réel.
    """
    try:
        data = json.loads(request.body)
        
        type_rappel = data.get('type_rappel', 'email')
        contenu = data.get('contenu', '')
        sujet = data.get('sujet', '')
        date_envoi = data.get('date_envoi')
        nb_destinataires = int(data.get('nb_destinataires', 1))
        
        # Validation complète
        valide, erreurs = valider_rappel_complet(
            type_rappel, sujet, contenu, date_envoi, nb_destinataires
        )
        
        # Conseils d'optimisation
        conseils = obtenir_conseils_optimisation(type_rappel, contenu, sujet)
        
        # Statistiques du contenu
        stats = _calculer_statistiques_contenu(type_rappel, contenu, sujet)
        
        return JsonResponse({
            'success': True,
            'valide': valide,
            'erreurs': [{'champ': err[0], 'message': err[1]} for err in erreurs],
            'conseils': conseils,
            'statistiques': stats
        }, encoder=ExtendedJSONEncoder)
...
@login_required
@require_http_methods(["POST"])
def api_previsualiser_rappel_intelligent(request):
    """
    API pour prévisualiser un rappel avec validation intelligente.
    """
    try:
        data = json.loads(request.body)
        ...
        valide, erreurs = valider_rappel_complet(type_rappel, sujet, contenu)
        
        if not valide:
            return JsonResponse({
                'success': False,
                'message': _("Contenu invalide"),
                'erreurs': [{'champ': err[0], 'message': err[1]} for err in erreurs]
            }, encoder=ExtendedJSONEncoder, status=400)
        
        # Récupérer la cotisation pour les variables
        cotisation = None
        if cotisation_id:
            try:
                cotisation = Cotisation.objects.get(pk=cotisation_id)
        ...
        if cotisation:
            contenu_genere = _generer_contenu_avec_variables(contenu, cotisation)
            sujet_genere = _generer_contenu_avec_variables(sujet, cotisation)
        else:
            contenu_genere = contenu
            sujet_genere = sujet
        
        # Adaptation selon le type
        apercu = _generer_apercu_formate(type_rappel, sujet_genere, contenu_genere)
        
        return JsonResponse({
            'success': True,
            'apercu': apercu,
            'type_rappel': type_rappel,
            'statistiques': _calculer_statistiques_contenu(type_rappel, contenu_genere, sujet_genere)
        }, encoder=ExtendedJSONEncoder)
...
@login_required
@require_http_methods(["GET"])
def api_suggestions_amelioration(request):
    """
    API pour obtenir des suggestions d'amélioration contextuelles.
    """
    type_rappel = request.GET.get('type_rappel', 'email')
    ...
    try:
        ...
        if not config:
            return JsonResponse({
                'success': False,
                'message': _("Type de rappel non supporté")
            }, encoder=ExtendedJSONEncoder, status=400)
        
        # Suggestions de longueur
        longueur = len(contenu)
        if longueur < config['longueur_optimale'] * 0.5:
            suggestions.append({
                'type': 'longueur',
                'priorite': 'haute',
                'titre': _("Contenu trop court"),
                'message': _("Ajoutez plus de détails pour un impact optimal"),
                'action': _("Viser {} caractères").format(config['longueur_optimale'])
            })
        
        # Suggestions selon le niveau
        if niveau >= 3 and type_rappel != 'courrier':
            suggestions.append({
                'type': 'escalade',
                'priorite': 'moyenne',
                'titre': _("Niveau élevé détecté"),
                'message': _("Considérez un courrier officiel pour plus d'impact"),
                'action': _("Passer au courrier recommandé")
            })
        ...
        if type_rappel == 'sms':
            if longueur > 140:
                suggestions.append({
                    'type': 'optimisation',
                    'priorite': 'haute',
                    'titre': _("SMS proche de la limite"),
                    'message': _("Raccourcissez pour éviter la troncature"),
                    'action': _("Supprimer {} caractères").format(longueur - 140)
                })
        
        elif type_rappel == 'email':
            if 'http' in contenu and not any(mot in contenu.lower() for mot in ['cliquez', 'visitez', 'accédez']):
                suggestions.append({
                    'type': 'engagement',
                    'priorite': 'moyenne',
                    'titre': _("Lien sans appel à l'action"),
                    'message': _("Ajoutez un texte incitatif pour améliorer le taux de clic"),
                    'action': _("Ajouter 'Cliquez ici pour...'")
                })
        
        # Templates alternatifs
        templates_alternatifs = RappelTemplate.objects.filter(
            type_rappel=type_rappel,
            niveau_min__lte=niveau,
            niveau_max__gte=niveau,
            actif=True
        ).exclude(
            nom__icontains=request.GET.get('template_actuel', '')
        )[:3]
        
        if templates_alternatifs:
            suggestions.append({
                'type': 'template',
                'priorite': 'basse',
                'titre': _("Templates alternatifs disponibles"),
                'message': _("{} autres templates compatibles").format(len(templates_alternatifs)),
                'action': _("Explorer les alternatives"),
                'templates': [{'id': t.id, 'nom': t.nom} for t in templates_alternatifs]
            })
        ...
        logger.error(f"Erreur lors des suggestions: {str(e)}")
        return JsonResponse({
            'success': False,
            'message': str(e)
        }, encoder=ExtendedJSONEncoder, status=500)
...
@login_required
@require_http_methods(["POST"])
def api_optimiser_contenu_automatique(request):
    """
    API pour optimiser automatiquement le contenu selon le type.
    """
    try:
        data = json.loads(request.body)
        ...
        contenu_optimise = _optimiser_contenu_automatique(type_rappel, contenu, niveau)
        
        # Calculer les améliorations
        stats_avant = _calculer_statistiques_contenu(type_rappel, contenu)
        stats_apres = _calculer_statistiques_contenu(type_rappel, contenu_optimise)
        
        ameliorations = {
            'longueur_changee': stats_apres['longueur'] - stats_avant['longueur'],
            'lisibilite_amelioree': stats_apres.get('score_lisibilite', 0) > stats_avant.get('score_lisibilite', 0),
            'optimisations_appliquees': []
        }
        
        # Détailler les optimisations appliquées
        if contenu != contenu_optimise:
            if type_rappel == 'sms':
                ameliorations['optimisations_appliquees'].extend([
                    'Raccourcissement pour SMS',
                    'Suppression caractères superflus',
                    'Optimisation emojis'
                ])
            elif type_rappel == 'email':
                ameliorations['optimisations_appliquees'].extend([
                    'Amélioration structure',
                    'Optimisation appels à l\'action'
                ])
        
        return JsonResponse({
            'success': True,
            'contenu_original': contenu,
            'contenu_optimise': contenu_optimise,
...
def _calculer_statistiques_contenu(type_rappel, contenu, sujet=""):
    """
    Calcule les statistiques d'un contenu selon son type.
    """
    stats = {
        'longueur': len(contenu),
    ...
    if config:
        stats.update({
            ...
            'pourcentage_optimal': (stats['longueur'] / config['longueur_optimale']) * 100,
            'dans_limites': config['longueur_min'] <= stats['longueur'] <= config['longueur_max']
        })
    
    # Statistiques spécifiques par type
    if type_rappel == 'sms':
        # Calculer longueur réelle SMS (emojis comptent double)
        import re
        ...
        stats.update({
            'longueur_reelle_sms': stats['longueur'] + nb_emojis,
            'nb_emojis': nb_emojis,
            'caracteres_restants': 160 - (stats['longueur'] + nb_emojis)
        })
    
    elif type_rappel == 'email':
        # Statistiques email
        nb_liens = contenu.count('http')
        stats.update({
            'nb_liens': nb_liens,
            'longueur_sujet': len(sujet),
            'contient_html': '<' in contenu and '>' in contenu
        })
    
    elif type_rappel == 'courrier':
        # Statistiques courrier
        stats.update({
            'paragraphes': len([p for p in contenu.split('\n\n') if p.strip()]),
            'format_formel': all(mot in contenu.lower() for mot in ['objet', 'madame', 'monsieur'])
        })
    
    return stats


def _generer_contenu_avec_variables(contenu, cotisation):
    """
    Remplace les variables dans le contenu avec les données de la cotisation.
    """
    if not cotisation:
        return contenu
    ...
    contenu_genere = contenu
    for var, valeur in variables.items():
        contenu_genere = contenu_genere.replace(f'{{{var}}}', str(valeur))
    
    return contenu_genere


def _generer_apercu_formate(type_rappel, sujet, contenu):
    """
    Génère un aperçu formaté selon le type de rappel.
    """
    apercu = {
        'type': type_rappel,
        'sujet': sujet,
    ...
    elif type_rappel == 'sms':
        apercu.update({
            'format': 'mobile',
            'apercu_mobile': f"""
                <div style="max-width: 300px; margin: 0 auto; background: #007bff; color: white; 
        ...
        apercu.update({
            ...
            'apercu_lettre': f"""
                <div style="max-width: 600px; margin: 0 auto; background: white; padding: 30px; 
                           border: 1px solid #ddd; font-family: 'Times New Roman', serif;">
                    <div style="text-align: center; margin-bottom: 20px;">
                        <strong style="text-decoration: underline;">OBJET: {sujet}</strong>
                    </div>
...
def _optimiser_contenu_automatique(type_rappel, contenu, niveau):
    """
    Optimise automatiquement le contenu selon le type et le niveau.
    """
    config = TypeRappelConfig.get_config(type_rappel)
    if not config:
        return contenu
    
    contenu_optimise = contenu
    
    # Optimisations communes
    contenu_optimise = contenu_optimise.strip()
    
    # Optimisations spécifiques par type
    if type_rappel == 'sms':
        # Raccourcir pour SMS
        if len(contenu_optimise) > config['longueur_max']:
            ...
            for mot, abbrev in abreviations.items():
                contenu_optimise = contenu_optimise.replace(mot, abbrev)
        
        # Supprimer formules de politesse longues
        if niveau <= 2:  # Seulement pour niveaux pas trop formels
            contenu_optimise = contenu_optimise.replace('Nous vous remercions', 'Merci')
            contenu_optimise = contenu_optimise.replace('de bien vouloir', 'de')
    
    elif type_rappel == 'email':
        # Améliorer structure email
        if not contenu_optimise.startswith(('Bonjour', 'Cher', 'Chère')):
            contenu_optimise = f"Bonjour,\n\n{contenu_optimise}"
        
        if not contenu_optimise.endswith(('Cordialement', 'Bien à vous', 'Salutations')):
            contenu_optimise = f"{contenu_optimise}\n\nCordialement"
    
    elif type_rappel == 'courrier':
        # Standardiser format courrier
        if 'Objet :' not in contenu_optimise and 'OBJET :' not in contenu_optimise:
            contenu_optimise = f"Objet : Rappel de cotisation\n\n{contenu_optimise}"
        
        if not any(formule in contenu_optimise for formule in ['Veuillez agréer', 'salutations']):
            contenu_optimise = f"{contenu_optimise}\n\nVeuillez agréer, Madame, Monsieur, l'expression de nos salutations distinguées."
    
    return contenu_optimise
